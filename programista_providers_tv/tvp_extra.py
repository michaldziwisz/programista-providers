...
from programista_providers_tv.teleman import TELEMAN_BASE, parse_teleman_stations


TVP_PROGRAM_URL = "https://www.tvp.pl/program-tv"


class TvpExtraTvProvider(ScheduleProvider):
    """
    Dostawca "brakujących" kanałów TVP, których nie ma w Telemanie.

    Obecnie obejmuje głównie kanały regionalne TVP3 oraz kanały takie jak
    TVP Wilno / TV Bielsat / ALFA TVP / ABC 2 (zależnie od tego, co znajduje
    się na stronie TVP i czego Teleman nie zwraca).
    """
    ...
    def display_name(self) -> str:
        return "Telewizja (TVP – brakujące kanały)"

    def list_sources(self, *, force_refresh: bool = False) -> list[Source]:
        # Use the same cache key as TelemanProvider so the request is shared.
        teleman_html = self._http.get_text(
            TELEMAN_BASE + "/",
        ...
        teleman_names = {name.casefold() for _slug, name in parse_teleman_stations(teleman_html) if name}
        ...
        missing = [st for st in stations if st.name.casefold() not in teleman_names]
        return [
            Source(provider_id=ProviderId(self.provider_id), id=SourceId(st.slug), name=st.name) for st in missing
        ]
